...
import com.avocado.streaming.utils.GenericAvroSerde;
...
public class StreamingApplication {
	...
	private static Integer commitInterval = 1000;
	...
	public static void main(String[] args) throws IOException {
		...
		final Serde<GenericRecord> genericAvroSerde = new GenericAvroSerde();
 		...
 		genericAvroSerde.configure(Collections.singletonMap(
				AbstractKafkaSchemaSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, "http://a5ef82e13fcbc44689f93c4924981608-494875664.ap-northeast-2.elb.amazonaws.com:8081"), isKeySerde
		);
...
//		streamsConfiguration.put(StreamsConfig.COMMIT_INTERVAL_MS_CONFIG, commitInterval);
		...
		//clickStream.foreach((key, value) -> System.out.println("Click Stream - Key: " + key + ", Value: " + value));
		clickStream.map(((key, value) -> KeyValue.pair(key, Result.newBuilder()
				.setUserId(value.get("userId").toString())
				.setAction(ActionType.CLICK)
				.build())))
				.to("test-result", Produced.with(longSerde, genericAvroSerde));
